...
export const holdRepository = {
            ...
            for (const item of items) {
                await client.query(
                    `INSERT INTO pos_held_order_items (
                        hold_id, product_id, product_name, product_sku,
    ...
    async listHoldOrders(
        ...
        if (filters.terminalId) {
            conditions.push(`h.terminal_id = $${paramIndex++}`);
            params.push(filters.terminalId);
        }

        if (!filters.includeExpired) {
            conditions.push(`(h.expires_at IS NULL OR h.expires_at > NOW())`);
        }
     ...
     */
    async getHoldOrderById(holdId: string): Promise<any | null> {
        const holdResult = await pool.query(
            "SELECT * FROM pos_held_orders WHERE id = $1 AND status = 'ACTIVE'",
            [holdId]
        );
     ...
     */
    async getHoldOrderByNumber(holdNumber: string): Promise<any | null> {
        const holdResult = await pool.query(
            "SELECT * FROM pos_held_orders WHERE hold_number = $1 AND status = 'ACTIVE'",
            [holdNumber]
        );
     ...
     */
    async deleteHoldOrder(holdId: string): Promise<void> {
        const result = await pool.query(
