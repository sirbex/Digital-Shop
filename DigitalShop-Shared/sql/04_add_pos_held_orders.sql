-- Migration: Add POS Held Orders Tables
-- Date: 2026
...
CREATE INDEX IF NOT EXISTS idx_pos_held_orders_user_id ON pos_held_orders(user_id);
CREATE INDEX IF NOT EXISTS idx_pos_held_orders_terminal_id ON pos_held_orders(terminal_id) WHERE terminal_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_pos_held_orders_created_at ON pos_held_orders(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pos_held_orders_expires_at ON pos_held_orders(expires_at) WHERE expires_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_pos_held_orders_hold_number ON pos_held_orders(hold_number);
CREATE INDEX IF NOT EXISTS idx_pos_held_orders_status ON pos_held_orders(status);

CREATE INDEX IF NOT EXISTS idx_pos_held_order_items_hold_id ON pos_held_order_items(hold_id);
CREATE INDEX IF NOT EXISTS idx_pos_held_order_items_product_id ON pos_held_order_items(product_id);
CREATE INDEX IF NOT EXISTS idx_pos_held_order_items_line_order ON pos_held_order_items(hold_id, line_order);
...
SELECT 'POS Held Orders tables created successfully!' AS status;
